...
write_plan() {
  ...
  echo 'Expected health: {"status":"ok","service":"civicclerk","version":"0.1.20","civiccore":"0.22.1"}'
...
expected='{"civiccore": "0.22.1", "service": "civicclerk", "status": "ok", "version": "0.1.20"}'
if [[ -z "${health}" ]]; then
...
if [[ "${health}" != "${expected}" ]]; then
  echo "Unexpected /health response: ${health}. Expected CivicClerk 0.1.20 with CivicCore 0.22.1." >&2
  exit 1
fi
