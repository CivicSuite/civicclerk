...
wheel_path="dist/civicclerk-1.0.4-py3-none-any.whl"
...
write_plan() {
  ...
  echo 'Expected health: {"status":"ok","service":"civicclerk","version":"1.0.4","civiccore":"1.2.0"}'
...
expected='{"civiccore": "1.2.0", "service": "civicclerk", "status": "ok", "version": "1.0.4"}'
if [[ -z "${health}" ]]; then
...
if [[ "${health}" != "${expected}" ]]; then
  echo "Unexpected /health response: ${health}. Expected CivicClerk 1.0.4 with CivicCore 1.2.0." >&2
  exit 1
fi
