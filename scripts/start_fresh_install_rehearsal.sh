...
wheel_path="dist/civicclerk-0.1.15-py3-none-any.whl"
...
write_plan() {
  ...
  echo 'Expected health: {"status":"ok","service":"civicclerk","version":"0.1.15","civiccore":"0.18.1"}'
...
expected='{"civiccore": "0.18.1", "service": "civicclerk", "status": "ok", "version": "0.1.15"}'
if [[ -z "${health}" ]]; then
...
if [[ "${health}" != "${expected}" ]]; then
  echo "Unexpected /health response: ${health}. Expected CivicClerk 0.1.15 with CivicCore 0.18.1." >&2
  exit 1
fi
