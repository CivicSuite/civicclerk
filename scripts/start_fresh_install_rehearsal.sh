...
wheel_path="dist/civicclerk-0.1.11-py3-none-any.whl"
...
write_plan() {
  ...
  echo 'Expected health: {"status":"ok","service":"civicclerk","version":"0.1.11","civiccore":"0.16.0"}'
...
expected='{"civiccore": "0.16.0", "service": "civicclerk", "status": "ok", "version": "0.1.11"}'
if [[ -z "${health}" ]]; then
...
if [[ "${health}" != "${expected}" ]]; then
  echo "Unexpected /health response: ${health}. Expected CivicClerk 0.1.11 with CivicCore 0.16.0." >&2
  exit 1
fi
