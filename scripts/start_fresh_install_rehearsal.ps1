param(
    [string]$WheelPath = "dist/civicclerk-0.1.13-py3-none-any.whl",
...
function Write-FreshInstallPlan {
    ...
    Write-Output "Expected health: {`"status`":`"ok`",`"service`":`"civicclerk`",`"version`":`"0.1.13`",`"civiccore`":`"0.17.0`"}"
...
try {
    ...
    if ($health.status -ne "ok" -or $health.service -ne "civicclerk" -or $health.version -ne "0.1.13" -or $health.civiccore -ne "0.17.0") {
        throw "Unexpected /health response: $($health | ConvertTo-Json -Compress). Expected CivicClerk 0.1.13 with CivicCore 0.17.0."
    }
