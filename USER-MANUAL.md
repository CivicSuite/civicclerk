...
Status: CivicClerk v0.1.17 runtime foundation manual
Version: `0.1.17`
...
ledgering, accessibility/browser QA gates, CivicClerk v0.1.17 release
artifacts, CivicCore v0.19.0-backed packet export bundles, a database-backed
...
```bash
python scripts/check_enterprise_installer_signing.py --artifact installer/windows/build/CivicClerk-0.1.17-Setup.exe
```
...
The runtime foundation now pins to the published `civiccore` v0.19.0 release wheel. Agenda intake uses
...
- `GET /health` returns `{"status":"ok","service":"civicclerk","version":"0.1.17","civiccore":"0.19.0"}`
...
`dist/civicclerk-0.1.17-release-handoff.zip`. If that zip already exists, the
...
reusable municipal IdP contract suite, the reusable backup-retention/off-host
contract suite, and unsigned-installer warning docs pass, while keeping
code-signing certificate proof, real municipal IdP tenant proof, real vendor API
proof, and city backup-retention/off-host storage approval as external proof
slots. Use `--require-external-proof` only when those city or
...
builds release artifacts and checksums, and publishes CivicClerk v0.1.17.
The current production-depth branch pairs CivicClerk with the published `civiccore` v0.19.0 release wheel
...
records-ready packet export bundles using CivicCore v0.19.0 provenance, export
