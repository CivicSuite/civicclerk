...
Status: CivicClerk v0.1.12 runtime foundation manual
Version: `0.1.12`
...
NovusAGENDA, accessibility/browser QA gates, CivicClerk v0.1.12 release
artifacts, CivicCore v0.16.0-backed packet export bundles, a database-backed
...
```bash
python scripts/check_enterprise_installer_signing.py --artifact installer/windows/build/CivicClerk-0.1.12-Setup.exe
```
...
- `GET /health` returns `{"status":"ok","service":"civicclerk","version":"0.1.12","civiccore":"0.16.0"}`
...
`dist/civicclerk-0.1.12-release-handoff.zip`. If that zip already exists, the
...
builds release artifacts and checksums, and publishes CivicClerk v0.1.12.
The current production-depth branch pairs CivicClerk with the published `civiccore` v0.16.0 release wheel
