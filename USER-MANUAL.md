...
Status: CivicClerk v0.1.13 runtime foundation manual
Version: `0.1.13`
...
```bash
python scripts/check_enterprise_installer_signing.py --artifact installer/windows/build/CivicClerk-0.1.13-Setup.exe
```
...
- `GET /health` returns `{"status":"ok","service":"civicclerk","version":"0.1.13","civiccore":"0.17.0"}`
...
`dist/civicclerk-0.1.13-release-handoff.zip`. If that zip already exists, the
...
builds release artifacts and checksums, and publishes CivicClerk v0.1.13.
