...
Status: CivicClerk v0.1.16 runtime foundation manual
Version: `0.1.16`
...
ledgering, accessibility/browser QA gates, CivicClerk v0.1.16 release
...
```bash
python scripts/check_enterprise_installer_signing.py --artifact installer/windows/build/CivicClerk-0.1.16-Setup.exe
```
...
- `GET /health` returns `{"status":"ok","service":"civicclerk","version":"0.1.16","civiccore":"0.18.1"}`
...
`dist/civicclerk-0.1.16-release-handoff.zip`. If that zip already exists, the
...
builds release artifacts and checksums, and publishes CivicClerk v0.1.16.
