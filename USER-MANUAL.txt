...
Status: CivicClerk v0.1.20 runtime foundation manual
Version: `0.1.20`
...
artifacts, CivicCore v0.22.0-backed packet export bundles, a database-backed
...
```bash
python scripts/check_enterprise_installer_signing.py --artifact installer/windows/build/CivicClerk-0.1.20-Setup.exe
```
...
The runtime foundation now pins to the published `civiccore` v0.22.0 release wheel. Agenda intake uses
...
- `GET /health` returns `{"status":"ok","service":"civicclerk","version":"0.1.20","civiccore":"0.22.0"}`
...
`dist/civicclerk-0.1.20-release-handoff.zip`. If that zip already exists, the
...
The current production-depth branch pairs CivicClerk with the published `civiccore` v0.22.0 release wheel
...
records-ready packet export bundles using CivicCore v0.22.0 provenance, export
