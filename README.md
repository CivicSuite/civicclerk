...
- `/staff/session` to report whether the live staff shell is in local open mode, OIDC-protected staff mode, bearer-protected staff mode, or trusted-header staff mode
- `/staff/auth-readiness` to report whether the current OIDC, bearer-token, or trusted-header staff auth contract is deployment-ready
...
- browser redirect sign-in/session UX on top of the OIDC staff-token validation
  mode
...
17. Follow GitHub issues and discussions as live sync, browser redirect sign-in UX, signed installer work, city-approved backup retention/off-host storage, and deployment hardening land.
