...
FREEZE_WHEEL_URL = (
    ...
    "civiccore-m1-freeze/civiccore-0.22.1-py3-none-any.whl"
)
...
def test_cleanroom_dockerfile_pins_base_image_cosign_and_target_commit() -> None:
    ...
    assert "ARG CIVICCORE_FREEZE_REF=civiccore-m1-freeze" in dockerfile
...
def test_cleanroom_runner_verifies_freeze_tag_and_full_civicclerk_gate() -> None:
    ...
    assert "civiccore-m1-freeze" in runner
...
def test_cleanroom_ci_uploads_evidence_artifact() -> None:
    ...
    assert "civiccore-m1-freeze" in workflow
