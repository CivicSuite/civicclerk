...
def test_pyproject_targets_published_civiccore_v0120_release_wheel() -> None:
    data = load_pyproject()
    dependencies = data["project"]["dependencies"]

    assert (
        "civiccore @ https://github.com/CivicSuite/civiccore/releases/download/v0.12.0/"
        "civiccore-0.12.0-py3-none-any.whl"
    ) in dependencies
    assert not any(
...
async def test_root_endpoint_explains_current_user_experience() -> None:
    ...
    assert payload["status"] == "v0.1.5 runtime foundation release"
    ...
    assert "v0.1.5" in payload["message"]
