...
def test_pyproject_targets_published_civiccore_v0130_release_wheel() -> None:
    data = load_pyproject()
    dependencies = data["project"]["dependencies"]

    assert (
        "civiccore @ https://github.com/CivicSuite/civiccore/releases/download/v0.13.0/"
        "civiccore-0.13.0-py3-none-any.whl"
    ) in dependencies
    assert not any(
