...
def test_pyproject_targets_published_civiccore_v0110_release_wheel() -> None:
    data = load_pyproject()
    dependencies = data["project"]["dependencies"]

    assert (
        "civiccore @ https://github.com/CivicSuite/civiccore/releases/download/v0.11.0/"
        "civiccore-0.11.0-py3-none-any.whl"
    ) in dependencies
    assert not any(
        "civiccore>=" in dep or "civiccore~=" in dep or dep == "civiccore==0.11.0"
...
async def test_root_endpoint_explains_current_user_experience() -> None:
    ...
    assert payload["status"] == "v0.1.4 runtime foundation release"
    ...
    assert "v0.1.4" in payload["message"]
