"""Milestone 12+ v0.1.12 release contract."""
...
def test_version_surfaces_are_synchronized_to_v0112() -> None:
    ...
    assert "Current version: `0.1.12`" in current_docs
    assert "Version: `0.1.12`" in current_docs
    assert "v0.1.12" in current_docs
    assert "0.1.0.dev0" not in current_docs
    assert "## [0.1.12] - 2026-05-01" in changelog
...
def test_release_workflow_and_docs_reference_v0112_release() -> None:
    ...
    assert "civicclerk v0.1.12" in docs
...
def test_fresh_install_rehearsal_script_prints_expected_plan() -> None:
    ...
    for expected in [
        ...
        "Expected health: {\"status\":\"ok\",\"service\":\"civicclerk\",\"version\":\"0.1.12\",\"civiccore\":\"0.16.0\"}",
...
def test_release_handoff_bundle_script_prints_expected_plan() -> None:
    ...
    for expected in [
        ...
        "Version: 0.1.12",
        "civicclerk-0.1.12-release-handoff.zip",
        "dist/civicclerk-0.1.12-py3-none-any.whl",
        "dist/civicclerk-0.1.12.tar.gz",
        "dist/SHA256SUMS.txt",
