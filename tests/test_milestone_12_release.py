"""Milestone 12+ v0.1.15 release contract."""
...
def test_version_surfaces_are_synchronized_to_v0114() -> None:
    ...
    assert "Current version: `0.1.15`" in current_docs
    assert "Version: `0.1.15`" in current_docs
    assert "v0.1.15" in current_docs
    assert "0.1.0.dev0" not in current_docs
    assert "## [0.1.15] - 2026-05-02" in changelog
...
async def test_health_endpoint_reports_release_version() -> None:
    ...
    assert response.json()["version"] == "0.1.15"
...
def test_release_workflow_and_docs_reference_v0114_release() -> None:
    ...
    assert "civicclerk v0.1.15" in docs
...
def test_fresh_install_rehearsal_script_prints_expected_plan() -> None:
    ...
    for expected in [
        ...
        "Expected health: {\"status\":\"ok\",\"service\":\"civicclerk\",\"version\":\"0.1.15\",\"civiccore\":\"0.18.1\"}",
...
def test_release_handoff_bundle_script_prints_expected_plan() -> None:
    ...
    for expected in [
        ...
        "Version: 0.1.15",
        "civicclerk-0.1.15-release-handoff.zip",
