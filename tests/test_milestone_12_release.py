...
def test_release_workflow_and_docs_reference_v100_release() -> None:
    ...
    assert "civiccore/releases/download/v1.0/civiccore-1.0.0-py3-none-any.whl" in workflow
    assert "civicclerk v1.0.0" in docs
    assert "published `civiccore` 1.0.0 wheel from the `v1.0` release asset" in docs
...
def test_fresh_install_rehearsal_script_prints_expected_plan() -> None:
    ...
    for expected in [
        ...
        "Expected health: {\"status\":\"ok\",\"service\":\"civicclerk\",\"version\":\"1.0.0\",\"civiccore\":\"1.0.0\"}",
