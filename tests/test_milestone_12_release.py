"""Milestone 12+ v0.1.9 release contract."""
...
def test_version_surfaces_are_synchronized_to_v019() -> None:
    ...
    assert "Current version: `0.1.9`" in current_docs
    assert "Version: `0.1.9`" in current_docs
    assert "v0.1.9" in current_docs
    assert "0.1.0.dev0" not in current_docs
    assert "## [0.1.9] - 2026-04-29" in changelog
...
def test_release_workflow_and_docs_reference_v019_release() -> None:
    ...
    assert "civiccore/releases/download/v0.15.0/civiccore-0.15.0-py3-none-any.whl" in workflow
    assert "civicclerk v0.1.9" in docs
    assert "published `civiccore` v0.15.0 release wheel" in docs
