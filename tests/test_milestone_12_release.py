"""Milestone 12+ v0.1.10 release contract."""
...
def test_version_surfaces_are_synchronized_to_v019() -> None:
    ...
    assert "Current version: `0.1.10`" in current_docs
    assert "Version: `0.1.10`" in current_docs
    assert "v0.1.10" in current_docs
    assert "0.1.0.dev0" not in current_docs
    assert "## [0.1.10] - 2026-04-29" in changelog
...
async def test_health_endpoint_reports_release_version() -> None:
    ...
    assert response.json()["version"] == "0.1.10"
...
def test_release_workflow_and_docs_reference_v019_release() -> None:
    ...
    assert "civicclerk v0.1.10" in docs
