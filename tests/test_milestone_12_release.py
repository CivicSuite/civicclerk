"""Milestone 12+ v0.1.18 release contract."""
...
def test_version_surfaces_are_synchronized_to_v0117() -> None:
    ...
    assert "Current version: `0.1.18`" in current_docs
    assert "Version: `0.1.18`" in current_docs
    assert "v0.1.18" in current_docs
    assert "0.1.0.dev0" not in current_docs
    assert "## [0.1.18] - 2026-05-02" in changelog
...
async def test_health_endpoint_reports_release_version() -> None:
    ...
    assert response.json()["version"] == "0.1.18"
...
def test_release_workflow_and_docs_reference_v0117_release() -> None:
    ...
    assert "civiccore/releases/download/v0.20.0/civiccore-0.20.0-py3-none-any.whl" in workflow
    assert "civicclerk v0.1.18" in docs
    assert "published `civiccore` v0.20.0 release wheel" in docs
...
def test_fresh_install_rehearsal_script_prints_expected_plan() -> None:
    ...
    for expected in [
        ...
        "Expected health: {\"status\":\"ok\",\"service\":\"civicclerk\",\"version\":\"0.1.18\",\"civiccore\":\"0.20.0\"}",
...
def test_release_handoff_bundle_script_prints_expected_plan() -> None:
    ...
    for expected in [
        ...
        "Version: 0.1.18",
        "civicclerk-0.1.18-release-handoff.zip",
        "dist/civicclerk-0.1.18-py3-none-any.whl",
        "dist/civicclerk-0.1.18.tar.gz",
        "dist/SHA256SUMS.txt",
