...
Depends on: published, Sigstore-attested `civiccore` 1.0.1 wheel from the `v1.0.1` release asset
...
- Python package metadata with the published `civiccore` 1.0.1 wheel from the `v1.0.1` release asset
...
- records-ready packet export bundles with CivicCore v1.0.1 manifests, SHA256 checksums, provenance, and hash-chained audit events
...
- CivicClerk v1.0.0 release gate and build artifacts
...
6. Confirm the fresh-machine smoke-check path:
   - `GET http://127.0.0.1:8776/health` must return `{"status":"ok","service":"civicclerk","version":"1.0.0","civiccore":"1.0.1"}`
